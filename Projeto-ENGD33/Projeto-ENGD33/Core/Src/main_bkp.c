...
/* USER CODE BEGIN PFP */
void Reset_Relays_Motores(void);
void Set_Relays_Motores(void);
void Reset_NRF24_CE(void);
void Set_NRF24_CE(void);
uint16_t Read_Corrente_Motor_1(void);
uint16_t Read_Corrente_Motor_2(void);
uint16_t Read_Corrente_Motor_3(void);
uint16_t Read_Voltagem_Bateria(void);
float Read_Temperatura_uP(void);
...
float Read_Temperatura_uP(void)
{
	...
	sConfig.Channel = TEMPSENSOR_CHANNEL;
	sConfig.Rank = 1;
	sConfig.SamplingTime = ADC_SAMPLETIME_112CYCLES;
	if (HAL_ADC_ConfigChannel(&hadc1, &sConfig) != HAL_OK)
	{
